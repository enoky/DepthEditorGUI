...
A local, single-file browser GUI for cleaning up **depth-map videos** (e.g. [DepthCrafter](https://github.com/Tencent/DepthCrafter) output) by tracking objects in the matching **RGB video** with [SAM 2.1](https://github.com/facebookresearch/sam2) and then dimming, brightening, compressing, or inpainting those objects in the depth video.
...
- The `brightness` treatment at 0.35–0.5 usually looks more natural than inpainting for objects that should stay visible but recede; `compress` with a low threshold flattens everything bright while leaving the rest of the frame untouched.
